...
def run_parser(url1, city, room, price_from, price_to):
    ...
    with open(f'data/{city}_{room}_{price_from}_{price_to}.json', 'w') as f:
...
async def search(message: types.Message):
    ...
    with open(f'data/{city}_{room}_{price_from}_{price_to}.json', 'r') as f:
        all_order = json.load(f)
    for order_key, order_value in list(all_order.items())[:count[0]]:
        db = []
        try:
            if order_value['photo']:
                for photo in order_value['photo']:
                    db.append(types.InputMediaPhoto(photo))
                await bot.send_media_group(message.chat.id, media=db)
    ...
    await message.answer(f'Оголошень: {count[0]} / {len(all_order)}', reply_markup=keyboard)


@dp.callback_query_handler(Text(equals='next'))
async def send_url(call: types.CallbackQuery):
    room = data_for_search[f'{call.from_user.id}']['room']
    price_from = data_for_search[f'{call.from_user.id}']['price_from']
    price_to = data_for_search[f'{call.from_user.id}']['price_to']
    city = url[0].split('/')[-2]

    with open(f'data/{city}_{room}_{price_from}_{price_to}.json', 'r') as f:
        all_order = json.load(f)

    for order_key, order_value in list(all_order.items())[count[0]:count[0] + 5]:
        db = []
        try:
            if order_value['photo']:
                for photo in order_value['photo']:
                    db.append(types.InputMediaPhoto(photo))
                await bot.send_media_group(call.message.chat.id, media=db)
            title = order_value['title']
            detail = order_value['detail']
            price = order_value['price']
    ...
    if count[0] < len(all_order) < count[0] + 5:
        count[0] += len(all_order) - count[0]
    else:
        count[0] += 5

    if count[0] <= len(all_order):
        await call.message.answer(f'Оголошення: {count[0]} / {len(all_order)}', reply_markup=keyboard)
    else:
